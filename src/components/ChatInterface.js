          ...
          <Fade key={idx} in={true}>
            ...
            >
              ...
              >
                ...
                {msg.role === 'assistant' && msg.context?.retrieved?.length > 0 && (
                  <Box sx={{ mt: 1, pt: 1, borderTop: '1px dashed', borderColor: 'divider' }}>
                    <Typography variant="caption" color="text.secondary" fontWeight={600}>
                      แหล่งอ้างอิง (top-{msg.context.retrieved.length}):
                    </Typography>
                    {msg.context.retrieved.map((c, i) => (
                      <Typography key={c.id} variant="caption" component="div" color="text.secondary" sx={{ mt: 0.5, fontSize: '0.7rem' }}>
                        [{i + 1}] {sourceLabelTH(c.source)} · score {c.score} · {c.preview}…
                      </Typography>
                    ))}
                  </Box>
                )}
